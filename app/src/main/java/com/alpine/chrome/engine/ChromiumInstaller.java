...
public class ChromiumInstaller {
    ...
    private void writeLaunchHelper() throws IOException {
        ...
        // NOTE: under proot, x11vnc needs -noshm (SysV SHM is unreliable).
        // Never use broad pkill -f chromium (matches script path and self-kills).
        String body = ""
                + "#!/bin/sh\n"
                + "export DISPLAY=:1\n"
                + "export HOME=/root\n"
                ...
                + "ls -la /tmp/.X11-unix/ 2>/dev/null || echo '[ac] no /tmp/.X11-unix yet'\n"
                + "\n"
                + "echo '[ac] starting x11vnc :5901 (noshm)'\n"
                ...
                + "  echo '[ac] ERROR: x11vnc failed to stay up — retry without -localhost'\n"
                + "  x11vnc -display :1 -rfbport 5901 -forever -shared -nopw \\\n"
                + "    -noshm -noxdamage -xkb -ncache 0 \\\n"
                + "    > /tmp/x11vnc.log 2>&1 &\n"
                + "  VNC_PID=$!\n"
                + "  sleep 2\n"
                + "  if [ -f /tmp/x11vnc.log ]; then echo '[ac] x11vnc retry log:'; cat /tmp/x11vnc.log; fi\n"
                + "  if ! kill -0 \"$VNC_PID\" 2>/dev/null; then\n"
                + "    echo '[ac] ERROR: x11vnc still failed'\n"
                + "    exit 1\n"
                + "  fi\n"
                + "fi\n"
                + "echo \"[ac] x11vnc pid=$VNC_PID\"\n"
                + "\n"
