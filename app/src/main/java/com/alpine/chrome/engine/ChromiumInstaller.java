...
public class ChromiumInstaller {
    ...
    private void writeLaunchHelper() throws IOException {
        ...
        String body = ""
                ...
                + "VNC_PID=$!\n"
                + "sleep 2\n"
                + "if [ -f /tmp/x11vnc.log ]; then echo '[ac] x11vnc log:'; cat /tmp/x11vnc.log; fi\n"
                ...
                + "  exit 1\n"
                + "fi\n"
                + "echo \"[ac] launching $CHROME (software GL)\"\n"
                // Avoid Vulkan/SwANGLE path that SIGTRAPs under proot; use mesa softpipe/llvmpipe
                + "exec \"$CHROME\" \\\n"
                + "  --no-sandbox \\\n"
                ...
                + "  --disable-features=Vulkan,VaapiVideoDecoder,UseSkiaRenderer \\\n"
                + "  --use-gl=disabled \\\n"
                + "  --ozone-platform=x11 \\\n"
                + "  --in-process-gpu \\\n"
                + "  --single-process \\\n"
